  ...
  return (
    <div onClick={onClick} className="card rl-job-card" style={{ padding: '24px 24px 20px', borderLeftColor: isUrgent ? '#c0152a' : '#d1d0cb', width: '100%', display: 'flex', flexDirection: 'column' }}>
      <style>{`
        ...
        .rl-job-title { font-family:'Rajdhani',sans-serif; font-size:20px; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; color:#212c42; line-height:1.15; transition:color 0.15s; }
        .rl-job-card:hover .rl-job-title { color:#c0152a; }
      `}</style>

      {/* Top row */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
        <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.18em', color: '#8299c0', background: '#f5f4f0', padding: '3px 8px' }}>
          {role.department}
        </span>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
        {[role.type, role.shift].filter(Boolean).map((tag: string) => (
          <span key={tag} style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#505e78', background: '#f5f4f0', border: '1px solid #e0deda', padding: '3px 10px', borderRadius: 0 }}>
            {tag}
          </span>
        ))}
        {role.openings > 1 && (
          <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#c0152a', background: 'rgba(192,21,42,0.06)', border: '1px solid rgba(192,21,42,0.2)', padding: '3px 10px', borderRadius: 0 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTop: '1px solid #f0efe9' }}>
        ...
        {isUrgent && (
          <span style={{ fontFamily: "'Rajdhani',sans-serif", fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#c0152a' }}>
            ● Urgent
          </span>
        )}
      </div>
