...
const STATUSES = ['All', 'Open', 'On Hold', 'Filled']
...
export default function JobBoard({ roles }: { roles: any[] }) {
  ...
  const filtered = useMemo(() => roles.filter(r => {
    ...
    if (urgent && r.priority !== 'High')             return false
    if (search) {
    ...
    return true
  }), [roles, status, dept, search, urgent])
  return (
        ...
        {STATUSES.map(s => (
          <button key={s} onClick={() => setStatus(s)}
            className={"px-4 py-2 rounded-full text-xs font-bold transition-colors " +
              (status === s ? "bg-navy-900 text-yellow-400" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>
            {s}
          </button>
        ))}
        <button onClick={() => setUrgent(!urgent)}
          className={"px-4 py-2 rounded-full text-xs font-bold " +
            (urgent ? "bg-red-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>
          🔴 Urgent
        </button>
        ...
        {DEPTS.map(d => (
          <button key={d} onClick={() => setDept(d)}
            className={"px-4 py-1.5 rounded-full text-xs transition-colors " +
              (dept === d ? "bg-red-600 text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-red-400")}>
            {d}
          </button>
        ))}
      </div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-serif text-navy-900">Open Positions</h2>
        <span className="text-sm text-gray-400">{filtered.length} role{filtered.length !== 1 ? 's' : ''} found</span>
      </div>
      {filtered.length === 0
        ? <div className="text-center py-20 text-gray-400">No roles match your search.</div>
